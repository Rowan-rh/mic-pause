...
1. 开始前检查工作区、remote URL 和项目分支约定。以 `develop` 作为开发基线；如果配置了 `upstream`，可以从已确认的上游获取基线更新。
2. 从基线创建 `codex/<简短主题>` 开发分支，在该分支完成实现、自审和适当验证。
3. 推送前确认目标仓库属于用户本人。只推送 `codex/<简短主题>` 到个人仓库；不得推送 `develop`、`main`、默认分支或 `upstream`，也不得将开发分支合入并推送 `develop`。
4. 远程归属或目标分支不明确时，先核实，不要尝试推送。

## Pull Request

1. PR 源分支为个人仓库中的 `codex/<简短主题>`；PR 目标仓库必须位于用户的 GitHub 命名空间 `Rowan-rh/*`，源分支与目标分支均在该个人仓库中；PR 目标分支使用 `main`。不得向其他组织/用户的原仓库或上游仓库提 PR；仓库或目标分支无法确认时，先核实再创建。
...
5. 创建 PR 不代表同意合并；除非用户明确要求，否则不要合并 PR。
